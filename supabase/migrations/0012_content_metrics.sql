...
-- ВНИМАНИЕ: `window` — зарезервированное слово Postgres, поэтому collect_window.
alter table public.content_metrics add column if not exists collect_window text;
...
create unique index if not exists content_metrics_pub_window
  on public.content_metrics (publication_id, collect_window);
