...
describe("upsertCompanies", () => {
  it("counts new vs updated by pre-existing phone", async () => {
    ...
    const r = await upsertCompanies(db, [SC("+998900000001"), SC("+998900000002"), SC("+998900000003")], "uz",
  ...
  it("is idempotent — 2nd run = all updated, same row except timestamps", async () => {
    ...
    const first = fakeDb([]);
    await upsertCompanies(first.db, input, "uz", "2026-01-01T00:00:00Z");
    const second = fakeDb(phones);
    const r2 = await upsertCompanies(second.db, input, "uz", "2026-01-02T00:00:00Z");
