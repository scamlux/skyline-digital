...
- **google** — основной и единственный рабочий источник (Places API, ключ в env,
  бесплатного кредита $200/мес хватает с запасом).
- **yandex** — коллектор реализован, но «Places HTTP API» (поиск по организациям)
  у Яндекса ПЛАТНЫЙ: от 195 000 ₽/год, бесплатного тарифа нет (проверено
  2026-09-03). Не подключаем.
